...
CustomSelect.prototype.initialize = function(element){
	...
	element.onItemsSlotChangedEventHandler = this.onItemsSlotChangedEventHandler.bind(element);
	...
	element.onDropdownClickEventHandler = this.onDropdownClickEventHandler.bind(element);
	element.onDropdownFocusOutEventHandler = this.onDropdownFocusOutEventHandler.bind(element);
	element.onOptionsClickEventHandler = this.onOptionsClickEventHandler.bind(element);
	element.onOptionsMutatedEventHandler = this.onOptionsMutatedEventHandler.bind(element);
	...
	element._observer = new MutationObserver(element.onOptionsMutatedEventHandler);
	
	element.itemsSlotElement = element.shadowRoot.querySelector('slot[name=items]');
	...
	element.itemsSlotElement.addEventListener('slotchange', element.onItemsSlotChangedEventHandler);
...
CustomSelect.prototype.initializeElements = function(element){
	...
	element.selectButtonElement = element.shadowRoot.querySelector('.selectButton');
...
CustomSelect.prototype.createStyleSlot = function(element){
	...
	slot.innerHTML = `
		...
		.selectButton {
			...
			position: absolute;
			border-width: 5px;
			border-color: var(--select-button-color) transparent transparent transparent;
			border-style: solid;
...
CustomSelect.prototype.attributeChangedCallback = function(name, oldValue, newValue){
	switch(name){
		case "value":
...
CustomSelect.prototype.updateValueRendering = function(){
	...
	this._value === undefined ? 
		this.valueElement.innerHTML = "&nbsp;" : this.valueElement.textContent = this._value;
};
...
CustomSelect.prototype.subscribe = function(){
	...
	dropdownElement.addEventListener('focusout', this.onDropdownFocusOutEventHandler);
...
CustomSelect.prototype.unSubscribe = function(){
	...
	dropdownElement.removeEventListener('click', this.onDropdownClickEventHandler);
	dropdownElement.removeEventListener('focusout', this.onDropdownFocusOutEventHandler);
...
CustomSelect = window.customElements.define("custom-select", CustomSelect);